...
function getOldestFromFirstSpecies(id) {
  const localizaAnimais = employees.find((employee) => employee.id === id).responsibleFor[0];
  // retorna o ID's do 1º animail do funcionário.

  // retorna um array de objs com os dados dos animais do funcionário.
  const animalMaisVelho = species.find((animal) => animal
    .id === localizaAnimais).residents.reduce((acumulador, valorCorrente) => {
    let retornaIdade;
